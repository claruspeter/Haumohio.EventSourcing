...
module StateStorage =
  ...
  let rec makeStateForDay (domain: 'D) (day:DateOnly) (projector: Projector<'K, 'S, 'E>) (container: EventSourcingContainer<'D>) =
    container.logger.LogWarning("Making state {Domain}{StateName} for {Today}", domain, typeof<'S>.Name, day)
    match loadStateForDay domain day container with 
    | Some state ->
      container.logger.LogWarning("Using previously calculated state")
      state
    | None ->
      let initial = 
        match findInitialEventDate domain container with 
        | Some x when x < day -> 
          container.logger.LogWarning("No state for previous day {Day} - projecting...", (day.AddDays -1))
          makeStateForDay domain (day.AddDays -1) projector container
        | Some _ ->
          container.logger.LogWarning("Initial event is {Day} - start from empty", day)
          State<'K, 'S>.empty
        | None ->
          container.logger.LogWarning("No initial event - start from empty")
          State<'K, 'S>.empty
  ...
  let makeState (domain: 'D) (projector: Projector<'K, 'S, 'E>) (container: EventSourcingContainer<'D>) =
    ...
    container.logger.LogWarning("Making state {Domain}.{StateName} for today {Today}", domain, typeof<'S>.Name, today)
    let firstEvent = findInitialEventDate domain container
    container.logger.LogWarning("Initial Event Date in {Domain}: {InitialDate}", domain, firstEvent)
    let initial = 
      ...
      | Some x when x < today -> 
        ...
        | Some state -> 
          container.logger.LogWarning("Starting from yesterday's state")
          state
        | None -> 
          container.logger.LogWarning("No state for yesterday - projecting...")
          makeStateForDay domain yesterday projector container
      | Some _ -> 
        container.logger.LogWarning("Initial event is today - start from empty")
        State<'K, 'S>.empty
      | None -> 
        container.logger.LogWarning("No initial event - start from empty")
        State<'K, 'S>.empty
